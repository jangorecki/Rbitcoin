...
options("Rbitcoin.antiddos.sec"=5)
```
...
Function gather all the wallet balances from specified sources, calculates its values in specified *value_currency* and returns following structure:
```{r wallet_manager_recent_str, echo=c(2)}
wallet_dt <- full_wallet_dt[wallet_id==max(wallet_id)] # recent wallet
str(wallet_dt)
```

We may do a simple plot of only recent balances
```{r wallet_manager_recent_plot, echo=c(1), dev="svg", fig.width = 7, fig.height = 5}
rbtc.plot(wallet_dt, type="recent")
wallet_dt <- full_wallet_dt # load full archive
```
