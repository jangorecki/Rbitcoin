...
market.api.query.bitstamp <- function(url, client_id, key, secret, req = list(), 
  ...
  if(url == 'https://www.bitstamp.net/api/cancel_order/') query_result_json <- toJSON(query_result_json) #bitstamp exception, cancel order does not return json object
  ...
  bitstamp_api_dict_cancel_order <- function(market = "bitstamp", base = NA_character_, quote = NA_character_, action = "cancel_order"){
    data.table(market = market, base = base, quote = quote, action = action,
               ...
               post_process = c(function(x) data.table(market = market, base = base, quote = quote,
                                                       ...
                                                       oid = NA_character_)),
               catch_market_error = c(function(x){
                 if(is.null(x)) stop(paste0("bitstamp cancel_order: error not handled by market: missing key object"),call.=FALSE)
                 else if('error' %in% names(x)) stop(paste0('bitstamp cancel_order: market error: ',x[['error']]),call.=FALSE)
                 else if(x != 'true') stop(paste0('bitstamp cancel_order: operation failed: market result other than \'true\''),call.=FALSE)
                 else x
               }))
  }
